...
def timing(description: str, logger=None) -> None:
    ...
    desc_str = f'{description}: {ellapsed_time}'
    if logger is None:
        print(desc_str)
    else:
...
def timeit(method):
    def timed(*args, **kw):
        ...
        ellapsed_time = time.time() - start
        if 'desc' in kw:
            desc_str = f"{method.__name__}:{ellapsed_time}: {kw['desc']}"
        else:
            desc_str = f'{method.__name__}:{ellapsed_time}'
        if logger is not None:
            logger.debug(desc_str)
        else:
            print(desc_str)
        return result
    return timed
...
def check_git_status():
    ...
    if platform.system() in ['Linux', 'Darwin'] and not os.path.isfile(
            '/.dockerenv'):
...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    m = model.module.model[-1] if hasattr(model,
                                          'module') else model.model[-1]  # Detect()
    ...
    print(
        'anchors/target = %.2f, Best Possible Recall (BPR) = %.4f' %
        (aat, bpr), end='')
    if bpr < 0.98:  # threshold to recompute
        ...
        if new_bpr > bpr:  # replace anchors
            new_anchors = torch.tensor(
                new_anchors,
                device=m.anchors.device).type_as(
                m.anchors)
...
def labels_to_class_weights(labels, nc=80):
    ...
    # weights = np.hstack([gpi * len(labels)  - weights.sum() * 9, weights *
    # 9]) ** 0.5  # prepend gridpoints to start
...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False,
             DIoU=False, CIoU=False, eps=1e-9):
...
def build_targets(p, targets, model):
    ...
    ai = torch.arange(
        na,
        device=targets.device).float().view(
        na,
        1).repeat(
            1,
...
def non_max_suppression(prediction, conf_thres=0.1,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if merge and (
            ...
            except BaseException:  # possible CUDA error https://github.com/ultralytics/yolov3/issues/1139
...
def coco_single_class_labels(path='../coco/labels/train2014/', label_class=43):
    # Makes single-class coco datasets. from utils.general import *;
...
def kmean_anchors(path='./data/coco128.yaml', n=9,
                  img_size=640, thr=4.0, gen=1000, verbose=True):
    ...
    def print_results(k):
        ...
        for i, x in enumerate(k):
            print('%i,%i' %
                  (round(x[0]), round(x[1])), end=',  ' if i < len(k) -
                  1 else '\n')  # use in *.cfg
        return k
    ...
    if isinstance(path, str):  # *.yaml file
        ...
        from wearing_detector.utils.datasets import LoadImagesAndLabels
...
def print_mutation(hyp, results, yaml_file='hyp_evolved.yaml', bucket=''):
    ...
    if bucket:
        os.system(
            'gsutil cp evolve.txt %s gs://%s' %
            (yaml_file, bucket))  # upload


def apply_classifier(x, model, img, im0):
    # applies a second stage classifier to yolov5 outputs
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            pred_cls2 = model(
                torch.Tensor(ims).to(
                    d.device)).argmax(1)  # classifier prediction
...
def plot_study_txt(f='study.txt', x=None):
    ...
    for f in ['study/study_coco_yolov5%s.txt' %
              x for x in ['s', 'm', 'l', 'x']]:
        y = np.loadtxt(
            f, dtype=np.float32, usecols=[
        ...
        ax2.plot(y[6, :j], y[3, :j] * 1E2, '.-', linewidth=2, markersize=8,
                 label=Path(f).stem.replace('study_coco_', '').replace('yolov5', 'YOLO'))
...
def plot_labels(labels, save_dir=''):
    ...
    try:
        ...
        sns.pairplot(x, corner=True, diag_kind='hist', kind='scatter', markers='o',
                     plot_kws=dict(
                         s=3, edgecolor=None, linewidth=1, alpha=0.02),
...
def plot_results(start=0, stop=0, bucket='', id=(), labels=(),
    ...
    # Plot training 'results*.txt' as seen in
