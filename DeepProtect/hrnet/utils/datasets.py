...
class InfiniteDataLoader(torch.utils.data.dataloader.DataLoader):
    ...
    def __init__(self, *args, **kwargs):
        ...
        object.__setattr__(
            self, 'batch_sampler', _RepeatSampler(
                self.batch_sampler))
...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        # cv2.imwrite(path + '.letterbox.jpg', 255 * img.transpose((1, 2,
        # 0))[:, :, ::-1])  # save letterbox image
...
class LoadWebcam:  # for inference
    def __init__(self, pipe=0, img_size=640):
        ...
        # pipe = 'http://wmccpinetop.axiscam.net/mjpg/video.mjpg'  # IP golf
        # camera

        # https://answers.opencv.org/question/215996/changing-gstreamer-pipeline-to-opencv-in-pythonsolved/
        # pipe = '"rtspsrc location="rtsp://username:password@192.168.1.64/1"
        # latency=10 ! appsink'  # GStreamer
        ...
        # protocols=GST_RTSP_LOWER_TRANS_TCP ! rtph264depay ! queue !
        # vaapih264dec ! videoconvert ! appsink"  # GStreamer
    ...
    def __next__(self):
        ...
        if not ret_val:
            raise StopIteration
        flip = True if isinstance(self.pipe, int) else False

        if flip:  # local camera
            img0 = cv2.flip(img0, 1)  # flip left-right

        img_path = 'webcam.jpg'
        return img_path, img0, flip
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.label_files = [x.replace(sa, sb, 1).replace(
            ...
            if l is not None and l.shape[0]:
                ...
                if create_datasubset and ns < 1E4:
                    ...
                    if exclude_classes not in l[:, 0]:
                        ns += 1
                        # shutil.copy(wearing_detector=self.img_files[i],
...
def box_candidates(box1, box2, wh_thr=2, ar_thr=20, area_thr=0.1):  # box1(4,n), box2(4,n)
    # Compute candidate boxes: box1 before augment, box2 after augment, wh_thr
    # (pixels), aspect_ratio_thr, area_ratio
    w1, h1 = box1[2] - box1[0], box1[3] - box1[1]
    w2, h2 = box2[2] - box2[0], box2[3] - box2[1]
    ar = np.maximum(w2 / (h2 + 1e-16), h2 / (w2 + 1e-16))  # aspect ratio
    return (w2 > wh_thr) & (h2 > wh_thr) & (
        w2 * h2 / (w1 * h1 + 1e-16) > area_thr) & (ar < ar_thr)  # candidates
...
def cutout(image, labels):
    ...
    def bbox_ioa(box1, box2):
        # Returns the intersection over box2 area given box1, box2. box1 is 4,
        # box2 is nx4. boxes are x1y1x2y2
        box2 = box2.transpose()
...
def reduce_img_size(path='path/images', img_size=1024):
    # creates a new ./images_reduced folder with reduced size images of
    # maximum size img_size
